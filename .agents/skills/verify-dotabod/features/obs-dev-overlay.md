...
- `overlay-assets` requires every rendered image and static browser resource to load without an HTTP error or runtime exception.
- `overlay-socket` records the Socket.IO requests and requires them to stay on the harness's closed localhost origin.
...
- **Inspect playing.** At 1920x1080, the overlay title, dev controls, visible win/loss and rank content, and loaded `playing.png` reference image are present.
- **Inspect picks.** Choose `Picks` and `Dire`; require the selected control values, `#pick-screen-hud`, `#picks-blocker-parent`, the picks reference image, and the underlying overlay after the reference image is hidden.
...
- The audit records every Socket.IO request and fails if any request leaves the expected localhost origin.
