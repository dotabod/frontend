...
- Run on a capable macOS/Linux developer machine or through `.github/workflows/verify-dotabod.yml` on a GitHub-hosted Linux runner.
- For GitHub runs, the branch under test is pushed and contains the same revision the workflow checks out.
...
- The overlay seed grants that disposable user an active Pro subscription and writes its generated user ID to the artifact directory.
- The overlay socket URL is a closed localhost port; the dev-mode pass never connects to the production GSI service.
- Never drive an existing server, browser, database, or user profile.

## Driving conventions

- Run `.agents/skills/verify-dotabod/scripts/run-profile-verification.sh` directly for a local pass. Matching pull requests run the workflow automatically; dispatch it manually only after the user has authorized the external action.
...
- [OBS dev overlay](./obs-dev-overlay.md) covers deterministic playing and picks states, dev images, chat messages, controls, OBS-sized layouts, and asset/runtime failures.
