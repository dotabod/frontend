...
const oklchToRgba = function oklchToRgba([lightness, chroma, hue = 0]: Oklch): Rgba {
...
const parseColor = function parseColor(value: string): Rgba {
  const variable = /^var\((--[^)]+)\)$/u.exec(value)?.[1]
  if (variable) {
  ...
  const rgb = /^rgba?\(([^)]+)\)$/u.exec(value)?.[1]?.split(',').map(Number)
...
const composite = function composite(foreground: Rgba, background: Rgba): Rgba {
...
const relativeLuminance = function relativeLuminance(color: Rgba) {
  const linear = color
    .slice(0, 3)
...
const contrastRatio = function contrastRatio(
  foreground: string,
  background: string,
  canvas?: string,
) {
