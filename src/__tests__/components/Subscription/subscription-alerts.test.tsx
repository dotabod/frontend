...
describe(SubscriptionAlerts, () => {
  ...
  it('shows credit guidance when a member has credit and no active plan', () => {
    ...
    expect(screen.getByText(/credit balance available/iu)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /apply credits now/iu })).toBeInTheDocument()
  })
  ...
  it('shows Update Payment action for PAST_DUE with Stripe customer and no stripeSubscriptionId', () => {
    ...
    expect(screen.getByRole('button', { name: /update payment/iu })).toBeInTheDocument()
  })
  ...
  it('shows Renew Now when a subscription is ending soon and actions are enabled', () => {
    ...
    expect(screen.getByRole('button', { name: /renew now/iu })).toBeInTheDocument()
  })
})
