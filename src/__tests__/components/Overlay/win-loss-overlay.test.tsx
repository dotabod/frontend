...
vi.mock('@/components/Overlay/wl/win-loss-card', () => ({
  default: ({
    wl,
  }: {
    wl: {
...
describe(WinLossOverlay, () => {
  ...
  it('saves a fixed challenge start date and duration together', () => {
    ...
    expect(screen.getByText(/ends automatically.*returns to per-stream/iu)).toBeInTheDocument()
  })
  ...
  it('keeps users per stream when no challenge is configured', () => {
    ...
    expect(
      screen.getByText(/No challenge active.*resets when a new stream starts/iu),
    ).toBeInTheDocument()
