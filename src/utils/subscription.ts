...
export const FEATURE_TIERS: Record<SettingKeys | ChatterSettingKeys, SubscriptionTier> = {
  ...
  commandStreamers: SUBSCRIPTION_TIERS.FREE,
  commandSuggestions: SUBSCRIPTION_TIERS.FREE,
  streamersNpSuffix: SUBSCRIPTION_TIERS.PRO,
  streamersAnnounce: SUBSCRIPTION_TIERS.PRO,
...
export function getCurrentPeriod(priceId?: string | null): PricePeriod {
  ...
  if (allPriceIds.some((price) => price.monthly === priceId)) {
    return 'monthly'
  }
  if (allPriceIds.some((price) => price.annual === priceId)) {
    return 'annual'
  }
  if (allPriceIds.some((price) => price.lifetime === priceId)) {
    return 'lifetime'
  }
