...
function fallbackHeroName(heroKey: string): string {
  return heroKey
...
export function buildMatchHistorySummary(groups: MatchResultGroup[]): {
  heroesPlayed: number
  losses: number
  matches: number
  winRate: number
...
export function buildHeroPerformance(
...
export function formatStreamerScore({
  direScore,
  myTeam,
  radiantScore,
}: {
