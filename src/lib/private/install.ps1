...
function Start-DotabodInstaller {
  ...
  try {
    ...
    $response = Invoke-WebRequest -UseBasicParsing -Uri $fileUrl -Method Head
    if ($null -eq $response) {
    ...
    $response = Invoke-WebRequest -UseBasicParsing -Uri $fileUrl -Method Get
    if ($null -eq $response) {
      Write-Log "Failed to get a response from $fileUrl" "ERROR"
      return $false
    }
...
$result = Start-DotabodInstaller

# Always pause at the end to ensure the window stays open
