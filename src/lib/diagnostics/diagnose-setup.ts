...
export const isCompleteDiagnosticPayload = function isCompleteDiagnosticPayload(
  byteLength: number,
): boolean {
  return byteLength === DIAGNOSTIC_PAYLOAD_BYTES
}

const isFresh = function isFresh(timestamp: string | null, now: number): boolean {
  if (!timestamp) {
    return false
  }
...
export const diagnoseSetup = function diagnoseSetup({
  browserProbe,
  gsiLastSeenAt,
