import type { CommandKeys, SettingKeys } from '@/lib/default-settings'
...
export interface WhatsNewEntry {
  ...
  // renders an inline toggle
  settingKey?: SettingKeys | ChatterSettingKeys
  // tri-state: checked = value ?? autoOptInNewFeatures
  followsNewFeatureMaster?: boolean
  // for settings that live on another page
  deepLink?: { path: string; section?: string }
  // e.g. '!set'
  command?: string
  // links to /blog/<slug>
  blogSlug?: string
...
export const groupWhatsNewByDate = function groupWhatsNewByDate(
  entries: WhatsNewEntry[],
): WhatsNewDateGroup[] {
...
export const entryToggleChecked = function entryToggleChecked(
  entry: WhatsNewEntry,
...
export const deepLinkLabel = function deepLinkLabel(deepLink: { path: string }): string {
  return (
