...
const RegisterPage: NextPageWithLayout = () => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      if (!session) {
        // Store intended plan in URL for after auth
        await signIn('twitch', { callbackUrl: `/register?plan=${plan}` })
        return
      }

      // User is authenticated, proceed to Stripe
      const priceId = getPriceId(plan as 'starter' | 'pro', 'monthly')
      const { url } = await createCheckoutSession(priceId, session.user.id)
