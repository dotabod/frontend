...
async function fetchLanguageProgress(projectId: string, languageId: string, token: string) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const languageProgress = await fetchLanguageProgress(projectId as string, languageId as string, token);
