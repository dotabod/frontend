...
async function updateMod(userId: string) {
  try {
    ...
    const response = await addModerator(
    ...
    return { message: 'Failed to add mod', error: error.message }
  }
}
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await updateMod(session.user.id)
